...
class BedrockService:
    ...
    def generate_outreach_message(self, donor_info: dict, state: str, language: str) -> str:
        ...
        prompt = (
            ...
            f"Requirements:\n"
            f"1. Write the message in TWO parts:\n"
            f"   - FIRST: Write the full message in English.\n"
            f"   - THEN: Add a divider line: '--- {language} ---'\n"
            f"   - THEN: Write the same message translated into {language} (use the native script of {language}, not transliteration, unless {language} is English).\n"
            f"2. If the preferred language is English, write only one English message (no divider or second section).\n"
            f"3. Keep the tone extremely warm, respectful, and polite.\n"
            f"4. Mention that there is an urgent blood requirement for a Thalassemia child who needs a regular transfusion.\n"
            f"5. The message must be WhatsApp-ready: short (within 3-4 sentences per language), actionable, and formatted nicely (you can use emojis and *bold* text for emphasis).\n"
            f"6. Do not include any placeholder text (like [Name] or [Phone]). Include a call to action asking them to reply to this message if they are available to donate.\n"
            f"7. Do not include any email headers, JSON wrapper, prefix or explanation. Output ONLY the raw message content."
        )

        # Build fallback templates in case Bedrock invocation fails
        english_fallback = f"Hello, urgent message from Blood Warriors. A Thalassemia patient child needs an urgent transfusion of *{blood_group}* blood. If you are available to donate, please reply to this message. Thank you!"

        regional_templates = {
            "Kannada": f"ನಮಸ್ಕಾರ, ಬ್ಲಡ್ ವಾರಿಯರ್ಸ್‌ನಿಂದ ತುರ್ತು ಸಂದೇಶ. ಥಲಸ್ಸೀಮಿಯಾ ಪೀಡಿತ ಮಗುವಿಗೆ ತುರ್ತಾಗಿ *{blood_group}* ರಕ್ತದ ಅವಶ್ಯಕತೆ ಇದೆ. ನೀವು ರಕ್ತದಾನ ಮಾಡಲು ಲಭ್ಯವಿದ್ದರೆ ದಯವಿಟ್ಟು ಈ ಸಂದೇಶಕ್ಕೆ ಪ್ರತಿಕ್ರಿಯಿಸಿ. ಧನ್ಯವಾದಗಳು!",
        ...
        if language == "English":
            fallback_msg = english_fallback
        else:
            regional = regional_templates.get(language, "")
            fallback_msg = f"{english_fallback}\n\n--- {language} ---\n\n{regional}" if regional else english_fallback
